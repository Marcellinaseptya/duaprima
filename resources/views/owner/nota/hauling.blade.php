...
<section class="content">
    <div class="container-fluid">
        ...
        <form method="GET" action="{{ route('owner.nota.hauling.index') }}" class="form-inline mb-3">
            ...
            <a href="{{ route('owner.nota.hauling.export', request()->all()) }}" target="_blank" class="btn btn-success">Export PDF</a>
        </form>
                        ...
                        <tr>
                            ...
                            <td>
                                @if($nota->status == 'pending')
                                    <form action="{{ route('owner.nota.hauling.approve', $nota->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('PUT')
                                        <button class="btn btn-success btn-sm" onclick="return confirm('Yakin ingin approve?')">Approve</button>
                                    </form>

                                    <form action="{{ route('owner.nota.hauling.reject', $nota->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('PUT')
                                        <button class="btn btn-danger btn-sm" onclick="return confirm('Yakin ingin menolak?')">Tolak</button>
                                    </form>
                                @else
                                    <span class="badge {{ $nota->status == 'approved' ? 'bg-success' : 'bg-danger' }}">
                                        {{ ucfirst($nota->status) }}
                                    </span>
                                @endif
                            </td>
